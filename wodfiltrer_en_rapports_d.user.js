...
// @namespace      world-of-dungeons.net
// @include        *.world-of-dungeons.*/wod/spiel//dungeon/report.php*
// @version        1.1.3
// @require        http://userscripts.org/scripts/source/102357.user.js?maxage=30&minage=1&method=show
// ==/UserScript==
...
else {if (document.getElementsByName('disabled')[0].value == 'details') {updateButton = document.getElementsByName('disabled');}}

for(var i = 0; i < updateButton.length; i++ )	{
	if (updateButton[i].value != 'details') {
...
function montre_filtrage () {
	if (document.getElementById('heros_options')) {
		...
		class_heros_options.innerHTML = '.vue_heros_options {position: fixed;background-color: blue;padding: 0.5em;z-index: 9999;}';
				...
				if (trList[i].getElementsByTagName('td')[1]) {
					...
					if (bln_annule) {
						...
						if (td2 && td2.getElementsByTagName('a')[0]){
							...
							if (td2.innerHTML.indexOf("themselves") >= 0) {
								for (var k = 0; k < name_persoD.length; k++ )	{
									if (name_persoD[k].indexOf("themselves") >= 0) name_persoD[k] = name_persoG;
								}
							}

						// cas spécial où il n'y a qu'un 'soi-même à droite et donc aucun lien (<a>)
						} else if (td2 && td2.innerHTML.indexOf("themselves") >= 0){
...
function get_names () {
	...
	div_heronames.forEach(function(element, index, array) {
		arr_noms['amis'][arr_noms['amis'].length] =  element.firstChild.innerHTML;
	});
	...
	for(var i = 0; i < table_noms.length; i++ )	{
		...
		if (str_quel_cote.innerHTML.indexOf("Attacker") >= 0) {bln_attaquant = true;} else {bln_attaquant = false;}
			...
			if (td && td.getElementsByTagName('a')[0]) {
				...
				if (bln_attaquant) {
					...
					if (nom && !in_array(nom, arr_noms['amis'])) {
						...
						if (td.getElementsByTagName('span').length > 0 && td.innerHTML.indexOf('rep_hero') >= 0) {
							// c'est un familier
